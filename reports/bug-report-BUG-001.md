...
The form briefly appears to submit, then the page reloads. The previously filled fields are cleared, returning the form to a blank state. No inline error messages are shown adjacent to the empty fields. The user has no clear indication of which fields were invalid and must re-enter all data from scratch.
...
This is a UX and functional issue that directly damages conversion rate on the checkout flow — the most business-critical path on any e-commerce storefront. Forcing a user to re-enter all form data after a validation failure introduces friction and increases cart abandonment risk. Additionally, the lack of per-field error messaging makes the storefront non-compliant with WCAG 2.1 accessibility guidelines (Success Criterion 3.3.1 — Error Identification).



The issue was reproduced consistently across 3 test runs.
...
\- \[ ] Screen recording: showing the submit to reload to blank form behaviour
...
2\. Adds a visible error message directly beneath each invalid field
